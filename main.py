...
if __name__ == '__main__':
    ...
    while (len(df)!=0):
        ...
        try:
            ...
            # logger.info(refs)
            if len(refs) ==0:
                df = next_article('!1.Warning: No refs',None,df)
                continue
        ...
        if success_download_flag:
            ...
            try:
                introduction = extract_introduction_from_pdf(abstract[0],file_path)
                ...
                # legends,imgs,info = extract_images_and_text_below(file_path,img_path)
                # logger.info(info)

            except Exception as e:
                df = next_article('!5.Error: Introdcution Extract',e,df)
                continue
            ...
            try:
                ...
                logger.info('Extract img Error!!')
                logger.info(f"错误类型: {type(e).__name__}",f"\n错误消息: {e}",f"\n堆栈跟踪:\n{traceback.format_exc()}")
                

            if introduction!= None:

                ###### Extract cites from introduction
                try:
                    citations, citation_nums = extract_cites(introduction,len(list(refs)))
                except Exception as e:

                    df = next_article("!6.Error: Extract cite error",e,df)
                    continue
                
                
                if len(citations) == 0:
                    logger.info("Error No citation found!")
                ...
                for i,citation in enumerate(citations):
                    ...
                    if cite_available:
                        thread = threading.Thread(target=process_gpt, args=(pp_num,citation_num,citation,shared_cites,lock))
                    ...
                    try:
                        ...
                        if has_pmc and pb_id!= -1:
                            ...
                            if pb_id not in df['pubmed_id'].values and pb_id not in pmc_record:
                                ...
                                logger.info('New Article Info: !!!\n',' Cite num:',cite,' pub_id:',pb_id,ref['Citation'])
                                
                            else:
                                logger.info('Error Article exist: ~~~n',' Cite num:',cite,' pub_id:',pb_id,ref['Citation'])
                                
                        else:
                            logger.info(' Article not in pmc: ~~~n',' Cite num:',cite,' pub_id:',pb_id,ref['Citation'])
